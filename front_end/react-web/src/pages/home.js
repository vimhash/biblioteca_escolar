import React from 'react';

const Home = () => {
  return (
    <body class="component_position">
        <nav class="text-black font-bold my-8" aria-label="Breadcrumb">
            <ol class="list-none p-0 inline-flex">
            <li class="flex items-center">
                <a href="/home">Inicio</a>
            </li>
            </ol>
        </nav>

        <main class="my-8">
            <hr />
            <div class="flex mb-4 h-56">
                <div class="w-1/4 h-48 flex-1 text-center px-4 py-2 m-2">
                    <div class=" rounded  rounded-t-lg bg-pink-300 overflow-hidden shadow max-w-xs my-2 pt-20">
                    <div class="flex justify-center -mt-8">
                        <img src="http://www.cavsi.com/preguntasrespuestas/images/que-es-usuario.jpg" class="rounded-full h-24 w-24 border-solid border-white border-2 -mt-5" />		
                    </div>
                    <div class="text-center px-3 pb-6 pt-2 bg-white pt-5 hover:bg-gray-400 ">
                        <h3 class="text-black text-sm bold uppercase font-sans">Administrador</h3>
                    </div>
                    </div>
                </div>
                <div class="w-1/4 h-48 flex-1 text-center px-4 py-2 m-2">
                    <div class=" rounded  rounded-t-lg bg-pink-300 overflow-hidden shadow max-w-xs my-2 pt-20">
                        <div class="flex justify-center -mt-8">
                            <img src="https://image.flaticon.com/icons/png/512/44/44948.png" class="rounded-full h-24 w-24 border-solid bg-white border-white border-2 -mt-5" />		
                        </div>
                        <div class="text-center px-3 pb-6 pt-2 bg-white pt-5 hover:bg-gray-400 ">
                            <h3 class="text-black text-sm bold uppercase font-sans">Estudiantes</h3>
                        </div>
                    </div>
                </div>

                <div class="flex mb-4 h-48">
                    <div class="w-1/4 h-48 flex-1 text-center px-4 py-2 m-2">
                        <div class=" rounded  rounded-t-lg bg-pink-300 overflow-hidden shadow max-w-xs my-2 pt-20">
                            <div class="flex justify-center -mt-8">
                                <img src="https://image.flaticon.com/icons/png/512/29/29302.png" class="rounded-full h-24 w-24 border-solid bg-white border-white border-2 -mt-5" />		
                            </div>
                            <div class="text-center px-3 pb-6 pt-2 bg-white pt-5 hover:bg-gray-400 ">
                                <h3 class="text-black text-sm bold uppercase font-sans">Libros</h3>
                            </div>
                        </div>
                    </div>
                    <div class="w-1/4 h-48 flex-1 text-center px-4 py-2 m-2">
                        <div class=" rounded  rounded-t-lg bg-pink-300 overflow-hidden shadow max-w-xs my-2 pt-20">
                            <div class="flex justify-center -mt-8">
                                <img src="data:image/png;base64," class="rounded-full h-24 w-24 border-solid bg-white border-white border-2 -mt-5" />
                            </div>
                            <div class="text-center px-3 pb-6 pt-2 bg-white pt-5 hover:bg-gray-400 ">
                                <h3 class="text-black text-sm bold uppercase font-sans">Categorias</h3>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </main>
    </body>
  );
}

export default Home;
